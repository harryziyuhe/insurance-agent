...
UNDERSTAND_SCHEMA = {
    ...
    "properties": {
        "full_name": {
            "type": "STRING",
            "nullable": True,
            "description": "The caller's full legal name, only if they stated it this turn.",
        },
        "dob": {
            "type": "STRING",
            "nullable": True,
            "description": "The caller's date of birth, only if they stated it this turn, in whatever format they said it.",
        },
        "phone": {
            "type": "STRING",
            "nullable": True,
            "description": "The caller's phone number, only if they stated it this turn.",
        },
        "email": {
            "type": "STRING",
            "nullable": True,
            "description": "The caller's email address, only if they stated it this turn.",
        },
        "id_last4": {
            "type": "STRING",
            "nullable": True,
            "description": "The last 4 digits of the caller's SSN or national ID, only if they stated it this turn.",
        },
        "policy_number": {
            "type": "STRING",
            "nullable": True,
            "description": "The caller's policy number, only if they stated it this turn.",
        },
        "intent_hint": {
            "type": "STRING",
            "nullable": True,
        ...
        "yes_no": {
            ...
            "enum": ["yes", "no"],
            "nullable": True,
            "description": "Set only if this message is clearly answering a yes/no question the agent just asked.",
        },
        "mentioned_document": {
            "type": "STRING",
            "nullable": True,
            "description": "The specific document name the caller mentions (e.g. 'pathology report'), only if one was named this turn.",
        },
        "lacks_document": {
            "type": "BOOLEAN",
            "description": "True only if the caller is clearly saying they don't have, can't find, or lost a document being discussed.",
        },
        "intent_category": {
            "type": "STRING",
            "nullable": True,
        ...
        "wants_different_case": {
            "type": "BOOLEAN",
            "description": "True only if the caller is clearly asking to switch to discussing a different claim.",
        },
        "is_done": {
            "type": "BOOLEAN",
            "description": (
        ...
        "in_scope": {
            "type": "BOOLEAN",
            "description": "False only if the message is unrelated to this insurance claim call (e.g. a general knowledge question). Default true.",
        },
        "emotion": {
            "type": "STRING",
            "enum": [
...
def _recent_history(session: SessionState) -> str:
    ...
    if not prior:
        return ""
    lines = [
        f"{'Caller' if t['role'] == 'user' else 'Agent'}: {t['text']}" for t in prior
    ]
    return (
        "Recent conversation, for context only — do not extract fields from these lines:\n"
        + "\n".join(lines)
        + "\n\n"
    )
