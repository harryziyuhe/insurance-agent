...
def handle(session: SessionState, user_text: str, extracted: ExtractedTurn) -> str:
    ...
    if extracted.is_done or _is_done(user_text):
        ...
        next_action = (
            "tell the caller you'll wrap up with a quick summary before you go"
        )
    ...
    if intent == "status_inquiry":
        ...
        next_action = (
            "tell the caller you checked and this is the claim's current status"
        )
        ...
        return respond(
            facts, next_action, extracted.emotion, claim=claim, session=session
        )
...
def _denial_reply(claim: dict, emotion: str, session: SessionState) -> str:
    ...
    next_action = (
        "explain why the claim was denied and what's needed to appeal, if anything"
    )
    return respond(facts, next_action, emotion, claim=claim, session=session)


def _document_reply(
    claim: dict, user_text: str, extracted: ExtractedTurn, session: SessionState
) -> str:
    guidance = get_document_guideline(
        claim,
        user_text,
        "document_submission",
        extracted.mentioned_document,
        extracted.lacks_document,
    )
...
def _wants_different_case(text: str) -> bool:
    t = text.lower()
    mentions_another = any(
        w in t for w in ["another claim", "different claim", "other claim"]
    )
